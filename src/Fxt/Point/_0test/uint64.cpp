...
static size_t stateHeapMemory_[ELEM_SIZE_AS_SIZET( sizeof( Uint64::StateBlock_T ) ) * MAX_POINTS];
...
TEST_CASE( "Uint64" )
{
    ...
    Uint64* apple = new(std::nothrow) Uint64( db, APPLE_ID, APPLE_LABEL, stateHeap );
    REQUIRE( apple );
    Uint64* orange = new(std::nothrow) Uint64( db, ORANGE_ID, ORANGE_LABEL, stateHeap, ORANGE_INIT_VAL );
    REQUIRE( orange );

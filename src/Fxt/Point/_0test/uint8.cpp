...
static size_t stateHeapMemory_[(sizeof( Uint8::StateBlock_T ) * MAX_POINTS + sizeof( size_t ) - 1) / sizeof( size_t )];
...
TEST_CASE( "Uint8" )
{
    ...
    Uint8* apple = new(std::nothrow) Uint8( APPLE_ID, APPLE_LABEL, stateHeap );
    REQUIRE( apple );
    Uint8* orange = new(std::nothrow) Uint8( ORANGE_ID, ORANGE_LABEL, stateHeap, ORANGE_INIT_VAL );
    REQUIRE( orange );
    ...
    SECTION( "read" )
    {
        ...
        REQUIRE( apple->getStatefulMemorySize() >= (sizeof( uint8_t ) + sizeof(bool)*2) );
    }
    ...
    {
        apple->setInvalid();
        apple->maskOr( (uint8_t) 0xA5 );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (uint8_t) 0xA5 );
        
        apple->maskAnd( (uint8_t) 0xF0 );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (uint8_t) 0xA0 );

        apple->maskXor( (uint8_t) 0x8A );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (uint8_t) 0x2A );

...
static size_t stateHeapMemory_[ELEM_SIZE_AS_SIZET( sizeof( Uint32::StateBlock_T ) ) * MAX_POINTS];
...
TEST_CASE( "Uint32" )
{
    ...
    Uint32* apple = new(std::nothrow) Uint32( db, APPLE_ID, APPLE_LABEL, stateHeap );
    REQUIRE( apple );
    Uint32* orange = new(std::nothrow) Uint32( db, ORANGE_ID, ORANGE_LABEL, stateHeap, ORANGE_INIT_VAL );
    REQUIRE( orange );

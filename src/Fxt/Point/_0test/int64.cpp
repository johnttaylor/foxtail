...
static size_t stateHeapMemory_[(sizeof( Int64::StateBlock_T ) * MAX_POINTS + sizeof( size_t ) - 1) / sizeof( size_t )];
...
TEST_CASE( "Int64" )
{
    ...
    Int64* apple = new(std::nothrow) Int64( APPLE_ID, APPLE_LABEL, stateHeap );
    REQUIRE( apple );
    Int64* orange = new(std::nothrow) Int64( ORANGE_ID, ORANGE_LABEL, stateHeap, ORANGE_INIT_VAL );
    REQUIRE( orange );
    ...
    SECTION( "read" )
    {
        ...
        REQUIRE( apple->getStatefulMemorySize() >= (sizeof( int64_t ) + sizeof( bool ) * 2) );
    }
    ...
    {
        apple->setInvalid();
        apple->maskOr( (int64_t) 0xA5 );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (int64_t) 0xA5 );

        apple->maskAnd( (int64_t) 0xF0 );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (int64_t) 0xA0 );

        apple->maskXor( (int64_t) 0x8A );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (int64_t) 0x2A );

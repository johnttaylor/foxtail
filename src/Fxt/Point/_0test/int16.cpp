...
static size_t stateHeapMemory_[(sizeof( Int16::StateBlock_T ) * MAX_POINTS + sizeof( size_t ) - 1) / sizeof( size_t )];
...
TEST_CASE( "Int16" )
{
    ...
    Int16* apple = new(std::nothrow) Int16( APPLE_ID, APPLE_LABEL, stateHeap );
    REQUIRE( apple );
    Int16* orange = new(std::nothrow) Int16( ORANGE_ID, ORANGE_LABEL, stateHeap, ORANGE_INIT_VAL );
    REQUIRE( orange );
    ...
    SECTION( "read" )
    {
        ...
        REQUIRE( apple->getStatefulMemorySize() >= (sizeof( int16_t ) + sizeof(bool)*2) );
    }
    ...
    {
        apple->setInvalid();
        apple->maskOr( (int16_t) 0xA5 );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (int16_t) 0xA5 );
        
        apple->maskAnd( (int16_t) 0xF0 );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (int16_t) 0xA0 );

        apple->maskXor( (int16_t) 0x8A );
        REQUIRE( apple->read( value ) );
        REQUIRE( value == (int16_t) 0x2A );

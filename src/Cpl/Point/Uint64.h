#ifndef Cpl_Point_Uint64_h_
#define Cpl_Point_Uint64_h_
/*-----------------------------------------------------------------------------
...
public:
    ...
    virtual void write( Uint64& src, Cpl::Point::Api::LockRequest_T lockRequest = Cpl::Point::Api::eNO_REQUEST ) noexcept 
    {
        if ( src.isNotValid() )
        {
            setInvalid();
        ...
        {
            BasicInteger_<uint64_t>::write( src.m_data, lockRequest );
        }
    }

public:
    ///  See Cpl::Dm::ModelPoint.
    const char* getTypeAsText() const noexcept { return "Cpl::Point::Uint64"; }

    ///  See Cpl::Point::Api
    size_t getTotalSize() const noexcept { return sizeof( Uint64 ); }


public:
    /// Creates a concrete instance in the invalid state
    static Api* create( Cpl::Memory::Allocator& allocatorForPoints, uint32_t pointId ) { return new(allocatorForPoints.allocate( sizeof( Uint64 ) )) Uint64( pointId ); }

...
constexpr Int8::Id_T  appleId  = 0;
constexpr Int8::Id_T  orangeId = 1;

#define ORANGE_INIT_VAL 7

static Int8 apple_( appleId );
static Int8 orange_( orangeId, ORANGE_INIT_VAL );
...
TEST_CASE( "Int8" )
{
    ...
    SECTION( "read" )
    {
        ...
        REQUIRE( apple_.getSize() == sizeof( int8_t ) );
    }
    ...
    {
        apple_.setInvalid();
        value = apple_.maskOr( (int8_t)0xA5 );
        REQUIRE( value == (int8_t)0xA5 );
        value = apple_.maskAnd( (int8_t)0xF0 );
        REQUIRE( value == (int8_t)0xA0 );
        value = apple_.maskXor( (int8_t)0x8A );
